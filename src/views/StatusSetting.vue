        ...
        <div style="position: relative;">
            <div class="box box1">
                ...
                <div style="text-align: right;">
                    <button class="button-6" role="button" >{{$t(`Import Setting`)}}</button>
                    <button class="button-6" role="button" >{{$t(`Reset`)}}</button>
                    <button class="button-6" role="button" @click="restoreprogress()">{{$t(`Restore previous progress`)}}</button>
                </div>
                <div v-for="(ID_id,ID_index) in All_IDs" :key="ID_index">
                    ...
                    <li  style="text-align: left;padding-left:150px;font-size: 1.15rem" v-for="(id, index) in ID_id.IDs" :key="index">
                        {{$t(index)}}
                        &nbsp;
                        <div >
                            <select :id='ID_index+index' :v-model="ID_id.IDs" @change="id.uptied=getitemselected(ID_index+index),updateIDdata()">
                    ...
                    <li  style="text-align: left;padding-left:150px;font-size: 1.15rem" v-for="(id, index) in ID_id.EGOs" :key="index">
                        {{$t(index)}}
                        &nbsp;
                        <div >
                            <select :id='ID_index+index' :v-model="ID_id.EGOs" @change="id.uptied=getitemselected(ID_index+index),updateIDdata()">
                                <option value="0">{{$t(`Don't have`)}}</option>
                                <option value="1">{{$t(`Uptie`)}}1</option>
                                <option value="2">{{$t(`Uptie`)}}2</option>
                                <option value="3">{{$t(`Uptie`)}}3</option>
